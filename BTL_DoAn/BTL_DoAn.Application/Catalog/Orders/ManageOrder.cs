...
namespace BTL_DoAn.Application.Catalog.Orders
{
    public class ManageOrder : IManageOrder
    {
        private readonly BTL_DoAnDbContext _context;
        public ManageOrder(BTL_DoAnDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<int> Delete(int OrderId)
        {
            ...
            if (order == null) throw new BTL_DoAnException("Can not find cart");
        ...
        public async Task<OrderViewModel> GetById(int OrderId)
        {
            var query = (from o in _context.Orders
                         select o).ToList();
            var data = await _context.Orders.Where(x => x.OrderId == OrderId).Select(x => new OrderViewModel()
            {
                Id = x.OrderId,
                ShipName = x.ShipName,
                ShipNumberPhone = x.ShipNumberPhone,
